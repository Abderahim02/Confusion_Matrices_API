...
def check_rows_columns_equal(csv_file_path):
    ...
    if num_rows != num_columns:
        raise ValueError(f"Error in file '{csv_file_path}': Number of rows ({num_rows}) is not equal to the number of columns ({num_columns}).")
    return True
...
def check_elements_per_row(csv_file_path):
    with open(csv_file_path, 'r') as file:
        ...
        for i, row in enumerate(csv_reader, start=1):
            if len(row)-1 != num_columns:
                raise ValueError(f"Error in file '{csv_file_path}': Number of elements in row {i} ({len(row)-1}) is not consistent with the number of columns ({num_columns}).")
    return True
...
def check_row_equals_column_names(csv_file_path):
    with open(csv_file_path, 'r') as file:
        ...
        for i, row in enumerate(csv_reader, start=1):
            if row[0] != header[i]:
                raise ValueError(f"Error in file '{csv_file_path}': name of the row {i} does not match the corresponding column name ({header[i]}).")
    return True
...
def check_elements_are_numbers(csv_file_path):
    with open(csv_file_path, 'r') as file:
        ...
        for i, row in enumerate(csv_reader, start=1):
            for j, element in enumerate(row, start=1):
                if j != 1 and not element.replace('.', '', 1).isdigit():
                    raise ValueError(f"Error in file '{csv_file_path}': Element in row {i-1}, column {j-1} is not a number.")
    return True


csv_dir = "./res_tst"
@pytest.mark.parametrize("csv_file", [file for file in os.listdir(csv_dir) if file.endswith(".csv")])
def test_csv_file(csv_file):
    csv_file_path = os.path.join(csv_dir, csv_file)
    
    assert check_rows_columns_equal(csv_file_path)
    assert check_elements_per_row(csv_file_path)
    assert check_row_equals_column_names(csv_file_path)
    assert check_elements_are_numbers(csv_file_path)
